...
helm install redis bitnami/redis \
  --namespace helix-infra --create-namespace \
  --set image.registry="localhost:5000" \
  --set image.repository="ecr-public.aws.com/docker/library/redis" \
  --set image.tag="7.2.8-alpine" \
  --set global.security.allowInsecureImages=true \
  --wait # Add --wait to wait for deployment to complete
# --- Post-Deployment Checks (example, adjust as needed) ---
...
helm install loki grafana/loki \
  --namespace helix-infra --create-namespace \
  --version 3.5.0 \
  --values "$(pwd)/helm-values/loki-values.yaml" \
  --set image.registry="localhost:5000" \
  --set image.repository="grafana/loki" \
  --set image.tag="3.5.0" \
  --wait
# --- Post-Deployment Checks (example, adjust as needed) ---
# Wait for deployment: kubectl rollout status deployment/loki
...
echo '--- Deployment commands generation complete. Review /home/angel/helix-integration-kubernetes-platform/helm-deployment-commands.sh ---'
